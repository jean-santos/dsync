...
struct Args {
    ...
    #[structopt(
        short = "t",
        long = "tables",
        help = "comma separted filter tables, for exemple: users,posts"
    )]
...
fn main() {
    ...
    dsync::generate_files(args.input, args.output, GenerationConfig {
        default_table_options: TableOptions::default().autogenerated_columns(args.autogenerated_columns.unwrap_or(vec![]).iter().map(|t| t.as_str()).collect::<Vec<&str>>()),
